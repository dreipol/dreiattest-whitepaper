...
Structure
=========
On a high level dreiAttest consists of 2 components: a mobile library (implemented for iOS, Android, and Kotlin multiplatform) and a view decorator running on the server. Before a request can be sent the mobile library generates a key pair and registers it with the view decorator. During the registration process DeviceCheck / SafetyNet is used to attest that the key was generated by a genuine installation of the app. The view decorator is responsible for validating the attestation and storing the public key. Afterwards the private key is used to sign any requests sent to the server and the view decorator can verify that it was signed by a genuine installation.
